...
class LocalAppearance(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.prediction_activation = Activation(self.output_activation_fn, dtype='float32', name='prediction_activation')
...
class SpatialConfiguration(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.spatial_activation = Activation(self.output_activation_fn, dtype='float32', name='prediction_activation')
    ...
    def call(self, local_prediction_and_image, training, **kwargs):
        ...
        spatial_prediction_wo_sigmoid = node = self.upsampling(node, training=training)
        spatial_prediction = self.spatial_activation(node, training=training)
        return spatial_prediction, spatial_prediction_wo_sigmoid
...
class Final(tf.keras.Model):
    ...
    def __init__(self,
                 data_format='channels_first',
                 local_prediction_activation='none',
                 spatial_prediction_activation='none',
                 ):
        super(Final, self).__init__()
        self.data_format = data_format
        self.local_prediction_activation_fn, _ = activation_fn_output_kernel_initializer(local_prediction_activation, self.data_format)
        self.spatial_predicition_activation_fn, _ = activation_fn_output_kernel_initializer(spatial_prediction_activation, self.data_format)
        self.local_spatial_combination = Lambda(lambda x: self.merge_based_on_spatial_uncertainty(x[0], x[1]), name='local_spatial_combination')
        self.prediction_fn = Lambda(lambda x: tf.cast(x, tf.float32) / (tf.reduce_sum(tf.cast(x, tf.float32), axis=1 if self.data_format == 'channels_first' else -1) + 1e-8))

    def merge_based_on_spatial_uncertainty(self, local_wo_activation, spatial_wo_activation):
        local_wo_activation = tf.cast(local_wo_activation, tf.float32)
        spatial_wo_activation = tf.cast(spatial_wo_activation, tf.float32)
        local = self.local_prediction_activation_fn(local_wo_activation)
        spatial = self.spatial_predicition_activation_fn(spatial_wo_activation)

        def sigmoid_uncertainty(tensor_wo_sigmoid, data_format=None, exponent=1.0):  # Note: data_format needed to use alias for either sigmoid or softmax
            tensor_wo_sigmoid = tf.cast(tensor_wo_sigmoid, tf.float32)
            cross_entropy_max = -(0.5 * tf.math.log(0.5)) * 2
        ...
        spatial_uncertainty = sigmoid_uncertainty(spatial_wo_activation, self.data_format)
        return local * spatial_uncertainty + spatial * (1 - spatial_uncertainty)

    def call(self, local_spatial_prediction, training):
        local_prediction, spatial_prediction = local_spatial_prediction
        prediction_wo_activation = self.local_spatial_combination([local_prediction, spatial_prediction])
        prediction = self.prediction_fn(prediction_wo_activation, training=training)
        return prediction, prediction_wo_activation
